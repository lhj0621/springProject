...
<script>
    function find_text()
    {
        ...
        form1.action="/~sale44/ajax/lists";
		else		// 값이 있는 경우, text1 값 전달
        form1.action="/~sale44/ajax/lists/text1/" + form1.text1.value;
		form1.submit();
    }
	$(function(){
			$("#ajax_add").click(function(){
				...
				$.ajax({
					...
					complete: function(xhr, textStatus){
						...
						"<tr id='rowno"+no+"'>"+
							"<td>"+ no + "</td>" +
							"<td><a href='/~sale44/ajax/edit/no/"+ no +"<?=$tmp?>'><font color='blue'>"+name+"</font></a></td>"+
...
<tr id="rowno<?=$no; ?>">
    <td><?=$no; ?></td>
    <td><a href="/~sale44/ajax/edit/no/<?=$no ?><?=$tmp;?>"><?=$row->name44; ?></a></td>
	<td>
