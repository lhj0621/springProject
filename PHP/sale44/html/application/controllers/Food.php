<?
    class Food extends CI_Controller {
		...
		if ($text1=="") 
		$base_url = "/food/lists/page";               // $page_segment = 4;
		else 
		$base_url = "/food/lists/text1/$text1/page";    // $page_segment = 6;
		$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;

		$base_url = "/~sale44" . $base_url;

		$config["per_page"]	 = 4;                              // 페이지당 표시할 line 수
		$config["total_rows"] = $this->food_m->rowcount($text1);  // 전체 레코드개수 구하기
		...
		$data["text1"]=$text1;
        $data["list"] = $this->food_m->getlist($text1,$start,$limit); // 해당페이지 자료읽기
		...
		$data["text1"]=$text1;
		$data["page"]=$page;
		$data["row"] = $this->food_m->getrow($no);
		...
		$this->food_m->deleterow($no);
		redirect("/~sale44/food/lists/".$text1.$page);
		}
		...
		{
		$data["list"] = $this->food_m->getlist_category();
		 $this->load->view("main_header");
		 $this->load->view("food_add",$data);    // 입력화면 표시
         $this->load->view("main_footer");
		 }
		...
		if($picname)$data["pic44"]=$picname;
         ...
         $result=$this->food_m->insertrow($data); 

        redirect("/~sale44/food/lists/".$text1.$page);    //   목록화면으로 이동.
			}
		}
		...
		if($picname)$data["pic44"]=$picname;
         ...
         $result=$this->food_m->updaterow($data,$no);
        redirect("/~sale44/food/lists/".$text1 . $page);    //   목록화면으로 이동.
		}
		}
		public function call_upload()
		{
			$config['upload_path']	= './food_img';
			...
			$config['source_image'] = './food_img/' .$picname;
			$config['thumb_marker'] = '';
			$config['new_image'] = './food_img/thumb';
