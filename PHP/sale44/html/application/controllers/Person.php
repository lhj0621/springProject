<?
    class Person extends CI_Controller {
		...
		if ($text1=="") 
		$base_url = "/person/lists/page";               // $page_segment = 4;
		else 
		$base_url = "/person/lists/text1/$text1/page";    // $page_segment = 6;
		$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;

		$base_url = "/~sale44" . $base_url;

		$config["per_page"]	 = 5;                              // 페이지당 표시할 line 수
		$config["total_rows"] = $this->person_m->rowcount($text1);  // 전체 레코드개수 구하기
		...
		$data["text1"]=$text1;
        $data["list"] = $this->person_m->getlist($text1,$start,$limit); // 해당페이지 자료읽기
		...
		$data["text1"]=$text1;
		$data["page"]=$page;
		$data["row"] = $this->person_m->getrow($no);
		...
		$this->person_m->deleterow($no);
		redirect("/~sale44/person/lists/".$text1.$page);
		}
		...
		{
		 $this->load->view("main_header");
		 $this->load->view("person_add");    // 입력화면 표시
         $this->load->view("main_footer");
		 }
         ...
         $result=$this->person_m->insertrow($data); 

        redirect("/~sale44/person/lists/".$text1.$page);    //   목록화면으로 이동.
			}
		}
		...
		if ( $this->form_validation->run()==FALSE )
		{
         ...
         $result=$this->person_m->updaterow($data,$no);
        redirect("/~sale44/person/lists/".$text1 . $page);    //   목록화면으로 이동.
		}
		}
